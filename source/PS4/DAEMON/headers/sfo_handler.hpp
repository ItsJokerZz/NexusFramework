// https://raw.githubusercontent.com/LightningMods/Itemzflow/ab1c95a5ab4895cc949bb7b6908a9caa3c2725e5/itemzflow/include/sfo.hpp
...
public:
    ...
    template <>
    inline std::string operator[](SfoEntry *e)
    {
        std::string s;
        std::vector<u8> eData;
    ...
    template <typename T>
    T GetValueFor(const std::string key)
    {
    ...
    template <>
    u64 GetValueFor(const std::string key)
    {
        for (u32 i = 0; i < hdr->entryCount; i++)
        {
            SfoEntry *e = _ent(i); // &entries[i];
    ...
    // T must be integer
    template <typename T>
    inline T GetValue(SfoEntry *e, T defVal = ~0, bool *res = nullptr)
    {
        return T(GetValue(e, defVal, res));
    }

    template <typename T>
    inline T GetValue(u32 index, T defVal = ~0, bool *res = nullptr)
    {
        SfoEntry *e = _ent(index); // this->operator[](index);
        return T(GetValue(e, defVal, res));
    }
    ...
    void printEntries()
    {
        for (u32 i = 0; i < hdr->entryCount; i++)
        {
            ...
            log_message("SfoEntry[%d]: \"%20s\" fmt: 0x%X len: %d ", i, GetKey(e).c_str(), e->paramFmt, e->paramLen);
            if (isNum(e))
                log_message("0x%08X", GetValue(e)); // u32(this->operator[](e)));
            else if (isText(e))
                log_message("\"%s\"", GetString(e).c_str());
            else
                log_message("ERROR unknown fmt?");
        }
    }
};
