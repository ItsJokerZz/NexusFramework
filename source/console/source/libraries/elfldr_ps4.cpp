 ...
 **/
static int elfldr_set_heap_size(pid_t pid, uint32_t size) {
...
static int elfldr_rfork_entry(void *unused) {
  ...
  if (open("/dev/deci_stdin", O_RDONLY) < 0) {
    klog_perror("open stdin");
    return 0;
  }
  if (open("/dev/deci_stdout", O_WRONLY) < 0) {
    klog_perror("open stdout");
    return 0;
  }
  if (open("/dev/deci_stderr", O_WRONLY) < 0) {
    klog_perror("open stderr");
    return 0;
  }
...
pid_t elfldr_spawn(int stdio, char *const argv[], uint8_t *elf,
  ...
  if ((pid = rfork_thread(RFPROC | RFCFDG | RFMEM, (char *)stack + MB(2),
