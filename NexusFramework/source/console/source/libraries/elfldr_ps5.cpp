...
static int r_relative(elfldr_ctx_t *ctx, Elf64_Rela *rela) {
  intptr_t *loc = (intptr_t *)((char *)ctx->base_mirror + rela->r_offset);
  intptr_t val = (intptr_t)ctx->base_addr + rela->r_addend;
...
static int data_load(elfldr_ctx_t *ctx, Elf64_Phdr *phdr) {
  void *data = (char *)ctx->base_mirror + phdr->p_vaddr;

  if (phdr->p_memsz == 0 || phdr->p_filesz == 0) {
    return 0;
  }

  memcpy(data, (char *)ctx->elf + phdr->p_offset, phdr->p_filesz);
...
intptr_t elfldr_load(pid_t pid, uint8_t *elf) {
  ...
  if ((ctx.base_addr = pt_mmap(pid, ctx.base_addr, ctx.base_size, prot, flags,
                               -1, 0)) == -1) {
 ...
 **/
static int elfldr_prepare_exec(pid_t pid, uint8_t *elf) {
  intptr_t entry;
  intptr_t args;
  struct reg r;
 ...
 **/
static int elfldr_set_procname(pid_t pid, const char *name) {
  intptr_t buf;
...
int elfldr_raise_privileges(pid_t pid) {
  static const uint8_t caps[16] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
 ...
 **/
static int elfldr_set_heap_size(pid_t pid, ssize_t size) {
  ...
  if (pt_copyout(pid, sceProcParam + 56, &sceLibcParam, sizeof(sceLibcParam))) {
  ...
  if (pt_copyout(pid, sceLibcParam + 72, &Need_sceLibc, sizeof(Need_sceLibc))) {
    LOG_PERROR("pt_copyout");
    return -1;
  }
...
static int sys_budget_set(long budget) { return __syscall(0x23b, budget); }

static int elfldr_rfork_entry(void *unused) {
  if (sys_budget_set(0)) {
  ...
  char *argv[] = {(char *)SceSpZeroConf, nullptr};
  char *envp[] = {nullptr};
...
pid_t elfldr_spawn(int stdio, char *const argv[], uint8_t *elf,
  ...
  if ((pid = rfork_thread(RFPROC | RFCFDG | RFMEM,
                          (uint8_t *)stack + PAGE_SIZE - 8, elfldr_rfork_entry,
                          (void *)argv)) < 0) {
